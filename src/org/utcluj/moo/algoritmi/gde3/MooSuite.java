...
public class MooSuite extends Problem implements SimpleProblemForm
    ...
    public static final String P_DEBBIN = "debBin";
    public static final String P_DTLZ2 = "dtlz2";
    public static final String P_DTLZ2_7_OBIECTIVE = "DTLZ2_7_obiective";
    ...
    public static final int PROB_DEBBIN = 15;
    public static final int PROB_DTLZ2 = 16;
    public static final int PROB_DTLZ2_7_OBIECTIVE = 17;
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        else if( wp.compareTo( P_DEBBIN) == 0)
            problemType = PROB_DEBBIN;
        else if( wp.compareTo( P_DTLZ2) == 0)
            problemType = PROB_DTLZ2;
        else if( wp.compareTo( P_DTLZ2_7_OBIECTIVE) == 0)
            problemType = PROB_DTLZ2_7_OBIECTIVE;
    ...
    private static final double PI_PE_2 = Math.PI/2;//dtlz2
    ...
    public void evaluate(final EvolutionState state,
        ...
        switch(problemType)
            ...
            case PROB_DEBBIN:
            	...
            	aux1 = - Math.pow(((genome[1]-0.2)/0.004), 2);
            	aux2 = - Math.pow(((genome[1]-0.6)/0.4), 2);
            	sum = 2-Math.exp(aux1)-0.8*Math.exp(aux2);
            	h = sum/f;
            	objectives[1] = (float)h;
        	break;
            case PROB_ZDT1:
                f = genome[0];
                objectives[0] = (float)f;
            ...
            case PROB_DTLZ2:
    			if (!(ind instanceof DoubleVectorIndividual)) {
    				System.err
    						.println("EROARE: individul tb sa fie de tipul DoubleVectorIndividual");
    				System.exit(1);
    			} else {
    				sum = 0; 
    				for (int i = 2; i < 12; i++)
    					sum += Math.pow(genome[i] - 0.5, 2);
    				g = 1 + sum;
    				objectives[0] = (float) (g * Math.cos(genome[0] * PI_PE_2)
    						* Math.cos(genome[1] * PI_PE_2));
    				objectives[1] = (float) (g * Math.cos(genome[0] * PI_PE_2)
    						* Math.sin(genome[1] * PI_PE_2));
    				objectives[2] = (float) (g * Math.sin(genome[0] * PI_PE_2));
    			}
    			break;
            case PROB_DTLZ2_7_OBIECTIVE:
            	if (!(ind instanceof DoubleVectorIndividual)) {
    				System.err
    						.println("EROARE: individul tb sa fie de tipul DoubleVectorIndividual");
    				System.exit(1);
    				...
    				objectives[0] = (float) (g * Math.cos(genome[0] * PI_PE_2)
    						* Math.cos(genome[1] * PI_PE_2)
    						* Math.cos(genome[2] * PI_PE_2)
    						* Math.cos(genome[3] * PI_PE_2)
    						* Math.cos(genome[4] * PI_PE_2)
    						* Math.cos(genome[5] * PI_PE_2));
    				objectives[1] = (float) (g * Math.cos(genome[0] * PI_PE_2)
    						* Math.cos(genome[1] * PI_PE_2)
    						* Math.cos(genome[2] * PI_PE_2)
    						* Math.cos(genome[3] * PI_PE_2)
    						* Math.cos(genome[4] * PI_PE_2)
    						* Math.sin(genome[5] * PI_PE_2));
    				objectives[2] =(float)  (g * Math.cos(genome[0] * PI_PE_2)
    						* Math.cos(genome[1] * PI_PE_2)
    						* Math.cos(genome[2] * PI_PE_2)
    						* Math.cos(genome[3] * PI_PE_2)
    						* Math.sin(genome[4] * PI_PE_2));
    				objectives[3] =(float) (g * Math.cos(genome[0] * PI_PE_2)
    						* Math.cos(genome[1] * PI_PE_2)
    						* Math.cos(genome[2] * PI_PE_2)
    						* Math.sin(genome[3] * PI_PE_2));
    				objectives[4] =(float) ( g * Math.cos(genome[0] * PI_PE_2)
    						* Math.cos(genome[1] * PI_PE_2)
    						* Math.sin(genome[2] * PI_PE_2));
    				objectives[5] =(float) ( g * Math.cos(genome[0] * PI_PE_2)
    						* Math.sin(genome[1] * PI_PE_2));
    				objectives[6] = (float) (g * Math.sin(genome[0] * PI_PE_2));
    			}
            	break;
            default:
