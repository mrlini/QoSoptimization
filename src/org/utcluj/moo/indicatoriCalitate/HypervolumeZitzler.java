...
public class HypervolumeZitzler {
	...
	private int wdf = 0; // weight distribution function according to zbt2007a
	private int hyp_ind_type = 0; // 0-fara scalare, usual hypervolume plus wdf
								// based
	...
	public HypervolumeZitzler(int dim, int[] obj, double[] nadir,
			double[] ideal, double[] reference_point) {
		super();
		this.dim = dim;
		this.obj = obj;
		this.nadir = nadir;
	...
	protected double calcHypervolum(double[][] front, double[] ref, int nrPct,
		...
		System.out.println("sunt in metoda");
		...
		while (n > 0) {
			...
			if (nrOb < 3) {
				...
				ref[dim] = front[0][0];
				volTemp = front[0][0];
				System.out.println("pas recursiv: "+pas);pas++;
//				System.out.println(ref.length);
//				volTemp = calcHypercubIntegral(ref, dim, hyp_ind_type, ideal,
//						wdf, pctRef);
				volTemp = hypervolume(ref, dim);
